...
.Mémoire physique et virtuelle (source icon:wikipedia-w[link="https://commons.wikimedia.org/wiki/File:Memoire_virtuelle.svg?uselang=fr"])
image::virtmem.svg[Mémoire virtuelle,303,480]
ifdef::backend-revealjs[]
--
endif::[]
...
image::memlayout.png[Mémoire d'un processus,201,597]
ifdef::backend-revealjs[]
--
endif::[]
...
* Est composée d'un cadre (_frame_) pour chaque fonction
...
+
WARNING: Toute variable allouée sur la pile dans une fonction n'est plus accessible après la fin de la fonction
* Sa taille est réservée statiquement
+
WARNING: La taille de la pile ne peut pas être modifiée à l'exécution (dynamiquement)

== Problèmes et erreurs liés à la mémoire
* De nombreuses erreurs peuvent conduire à des problèmes d'utilisation de la mémoire

.Références
* https://queue.acm.org/detail.cfm?id=3773095[Memory Safety for Skeptics], _Andrew Lilley Brinker_, ACM Queue 2025
