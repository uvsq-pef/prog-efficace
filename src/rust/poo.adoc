...
== Fonction associée
* Une https://doc.rust-lang.org/book/ch05-03-method-syntax.html#associated-functions[_fonction associée_] (_associated functions_) est une fonction définie dans le contexte d'une structure
* Il est possible de définir une telle fonction sans le paramètre `self`
** invoquée par l'intermédiaire du nom de la structure (`Rectangle::square(3)`)
...
** `Self` peut être utilisé pour référencer le type se trouvant après `impl`
...
* Un objet trait dispose d'une https://en.wikipedia.org/wiki/Virtual_method_table[_table de méthodes virtuelles_] (_vtable_)
...
* La https://en.wikipedia.org/wiki/Virtual_method_table[table des méthodes virtuelles] d'un objet trait permet de retarder le choix de la méthode à exécuter au moment de l'exécution
